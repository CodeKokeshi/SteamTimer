...
---------------------------------
Simple modern-looking timer window that displays elapsed time in
Days : Hours : Minutes : Seconds format. Intended to be launched
and kept running so a platform (e.g., Steam as a Non-Steam shortcut)
registers "play time" while you are actually playing a title on
another store.

DISCLAIMER: Artificially inflating playtime may violate Terms of
Service of certain platforms. Use responsibly. This tool performs
no network or platform API interaction; it only shows a timer.

Usage:
  python main.py [--offset-hours N] [--offset-seconds N]

Examples:
  python main.py --offset-hours 42
  python main.py --offset-seconds 3600

Build to EXE (after installing requirements & PyInstaller):
...
class MainWindow(QMainWindow):
	...
	def show_about(self):
		QMessageBox.information(
			...
			(
				f"{APP_TITLE}\n\n" 
				"Created by CodeKokeshi.\n\n" 
				"GitHub: https://github.com/CodeKokeshi/\n" 
				"itch.io: https://codekokeshi.itch.io/\n\n" 
				"A minimal timer to accumulate runtime (DD:HH:MM:SS).\n" 
